...
function $task(ctx, obj, isTX) {
    ...
    if (isTX) {
        ...
        var beginText = ctx.activeTx == 1 ? 'begin' : 'savepoint sp' + ctx.activeTx;

        return obj.none(beginText) // BEGIN;
            .then(function () {
                return invoke() // callback;
                    .then(function (data) {
                        ...
                        var rollbackText = ctx.activeTx == 1 ? 'rollback' : 'rollback to savepoint sp' + ctx.activeTx;
                        return obj.none(rollbackText); // ROLLBACK;
                    })
                    .then(function () {
                        if (success) {
